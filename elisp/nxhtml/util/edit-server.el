;; edit-server.el --- Emacs edit-server for Google Chrome
;;
...
;;
;; Last-Updated: 2010-06-17 Thu
;; URL: http://github.com/stsquad/emacs_chrome/raw/master/servers/edit-server.el
;; See also http://www.emacswiki.org/emacs/EmacsEchoServer
...
;;;###autoload
(define-minor-mode google-chrome-server-mode
  "Accept editing requests from Google Chrome."
  :group 'edit-server
  :global t
  (if google-chrome-server-mode
      (edit-server-start)
    (edit-server-stop)))
...
(defcustom edit-server-buffer-setup-hook nil
  "Hook run after creating edit server buffer.
...
When called interactively, use prefix arg to abort editing."
  ...
  (unless (or (not edit-server-proc)
              (eq (process-status edit-server-proc) 'closed))
